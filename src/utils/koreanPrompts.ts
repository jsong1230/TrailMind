 ...
 * 한글 반성 분석 결과 타입
 */
export interface KoreanReflectionAnalysis {
  핵심_요약: string;
  사실과_해석: {
    사실: string[];
    해석: string[];
 ...
 */
export function parseKoreanAIResponse(response: string): KoreanReflectionAnalysis | null {
  try {
    ...
    // JSON 파싱
    const parsed = JSON.parse(jsonText) as KoreanReflectionAnalysis;
    
    // 기본 검증
    if (
      typeof parsed.핵심_요약 === 'string' &&
      typeof parsed.사실과_해석 === 'object' &&
      Array.isArray(parsed.사실과_해석.사실) &&
      Array.isArray(parsed.사실과_해석.해석) &&
      Array.isArray(parsed.감정_신호) &&
      Array.isArray(parsed.관계_신호) &&
      typeof parsed.재해석 === 'string' &&
      typeof parsed.오늘의_질문 === 'string' &&
      typeof parsed.아주_작은_행동 === 'string'
    ) {
      return parsed;
    }
    ...
    console.error('Failed to parse Korean AI response:', error);
    return null;
  }
}
